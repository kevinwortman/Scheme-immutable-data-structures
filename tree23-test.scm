...
(let-values (((small-n medium-n large-n)
	      (if (not thorough)
		  (values 10 100 1000)
		  (values 1000 10000 (* 100 1000)))))

(define cmp number-comparator)
(define search (cute tree23-search cmp identity (constant #f) <> <>))
...
(test 'missing (tree23-delete cmp (constant 'missing) digits 5.5))
...
(loop ((for i (up-from 0 (to medium-n))))
  (test (iota i)
	(tree23->ordered-list
	 (tree23-scaffold-finish
...
(loop ((for i (up-from 0 (to small-n))))
  (let* ((n (+ medium-n (random medium-n)))
	 (sorted (iota n))
...
(let ((sorted (iota large-n))
  ...
  (let ((tree (ordered-list->tree23 sorted)))
    ...
    (timer "map/monotone" (tree23-map/nondecreasing add1 tree))
